...
const NovaReservaAluno = () => {
  ...
  // Estados para salas disponíveis
  ...
  // Recursos disponíveis para as salas
  const roomResources = [
  ...
  const validateForm = () => {
    ...
    if (!selectedRoom) {
      newErrors.room = "Selecione uma sala disponível";
    }
  ...
  const handleInputChange = (e) => {
    ...
    // Validação especial para o campo de data
    if (name === 'date' && value) {
      // Verificar se o ano tem mais de 4 dígitos
      const year = value.split('-')[0];
      if (year && year.length > 4) {
        // Não permitir a mudança se o ano tiver mais de 4 dígitos
        return;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const reservationData = {
        title: `Reserva - ${getSelectedProjectName()}`,
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        ...
        {successMessage && (
          ...
          <div className="flex gap-8">
            ...
            {/* Mini Calendário e Ações - Lado Direito */}
              ...
              <div className="bg-white rounded-lg shadow-sm border p-4">
                ...
                {availableRooms.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    <p className="text-sm">Nenhuma sala disponível neste bloco</p>
            </div>
                )}
            ...
            {selectedRoom && (
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="font-medium text-gray-800">{selectedRoom.name}</h4>
                        <p className="text-sm text-gray-600">{selectedRoom.capacity} lugares</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {selectedRoom.resources?.projector && <Monitor size={16} className="text-gray-500" />}
                        {selectedRoom.resources?.internet && <Wifi size={16} className="text-gray-500" />}
                        {selectedRoom.resources?.air_conditioning && <AirVent size={16} className="text-gray-500" />}
                      </div>
                    </div>
              </div>
            )}
